 ...
 * which side the photo sits on. Any gallery images left over once every
 * section has claimed one fall back to the same plain grid the case-study
 * page used before this existed.
 */
export function CaseStudyBodySections({ sections, gallery = [], title }: Props) {
  const leftoverGallery = gallery.slice(sections.length);
              ...
              <Reveal key={section.marker} delay={index * 0.1}>
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16 items-center">
                  ...
                  >
                    ...
                    <h2 className="font-display font-black text-xl md:text-2xl leading-8 tracking-normal uppercase text-brand-light mb-6">
                      {section.heading}
                    </h2>
      ...
      {leftoverGallery.length > 0 && (
        <section className="pl-6 sm:pl-24 lg:pl-48 pr-6 md:pr-10 lg:pr-24 pb-24">
          <div className="mx-auto max-w-7xl grid grid-cols-1 md:grid-cols-2 gap-6">
            {leftoverGallery.map((image, index) => (
              <div
                key={index}
                className="relative aspect-4/3 overflow-hidden rounded-4xl bg-brand-dark"
              >
                <Image
                  src={urlFor(image).width(1200).height(900).url()}
                  alt={`${title} — ${sections.length + index + 1}`}
                  fill
