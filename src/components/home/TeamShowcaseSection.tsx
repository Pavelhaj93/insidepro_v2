...
export function TeamShowcaseSection({
  ...
  heading = "Náš tým k vašim službám",
  teamMembers = [],
}: Props) {
        ...
        {teamMembers.map((member) => (
          <RevealItem key={member._id} className="group">
            <div className="relative aspect-4/5">
              <div className="absolute inset-0 rounded-4xl overflow-hidden rounded-br-none bg-brand-dark">
                {member.photo && (
                  <SanityImage
              ...
              <InvertedCorner className="absolute rotate-90 left-0 bottom-[25%] w-8 h-8 text-brand-black" />

              <div className="absolute bottom-0 left-0 w-[68%] h-[25%] bg-brand-black px-5 py-2 flex flex-col justify-center rounded-tr-4xl">
