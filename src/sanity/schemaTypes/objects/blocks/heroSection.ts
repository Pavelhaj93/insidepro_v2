...
export const heroSection = defineType({
  ...
  fields: [
    defineField({
      ...
      description: 'Always required as the poster frame — shown instantly while the background video (if any) loads in behind it.',
    }),
    defineField({
      ...
      description: 'Short, muted, looping background clip, landscape framing (e.g. 16:9). Used on all screens unless a mobile-specific clip is set below. Compress before uploading — see docs/features for the recommended ffmpeg command. If empty, the background image is used alone.',
    }),
    defineField({
      ...
      type: 'file',
      options: { accept: 'video/mp4,video/webm' },
      description: 'Optional portrait-framed clip (e.g. 9:16), swapped in automatically on narrow screens instead of cropping the desktop video. Falls back to the video above if left empty.',
    }),
